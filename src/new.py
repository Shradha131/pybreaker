...
class CircuitBreaker(object):
    ...
    def __init__(self, err_threshold=0.5, request_volume_window=20, fail_max=5, reset_timeout=60, exclude=None,
        ...
        self._state_storage._fail_counter = 0
        self._state_storage._success_counter = 0
        self._state_storage._error_rate = 1
        self._state_storage._total_calls_counter = 0
...
class CircuitMemoryStorage(CircuitBreakerStorage):
    ...
    @property
    def total_calls_counter(self):
        """
        Returns the current value of the failure counter.
        """
        return self._success_counter + self._fail_counter
...
class CircuitBreakerState(object):
    ...
    def before_call(self, func, *args, **kwargs):
        ...
        """
        print("before_call : Success Counter : {}".format(self._breaker.success_counter))
        pass
    ...
    def on_success(self):
        ...
        """
        self._breaker._inc_counter_success()
        print("on_success : Success Counter : {}".format(self._breaker.success_counter))
        pass
...
class CircuitClosedState(CircuitBreakerState):
    ...
    def __init__(self, cb, prev_state=None, notify=False):
        ...
        if notify:
            ...
            print self._breaker._state_storage.total_calls_counter
    ...
    def on_failure(self, exc):
        ...
        self._breaker._state_storage.err_rate = self._breaker._state_storage.fail_counter / self._breaker._state_storage.total_calls_counter

        print 'close state fail'

        print self._breaker._state_storage.err_rate
    ...
    def on_success(self):
        ...
        """
        print 'on success in close state'
        print self._breaker._state_storage.fail_counter
        print self._breaker._state_storage.success_counter
        print self._breaker._state_storage.total_calls_counter
        self._breaker._inc_counter_success()
...
class CircuitOpenState(CircuitBreakerState):
    ...
    def before_call(self, func, *args, **kwargs):
        ...
        print 'before call:  CircuitOpenState'
        print self._breaker._state_storage.fail_counter
        print self._breaker._state_storage.success_counter
        print self._breaker._state_storage.total_calls_counter

        if opened_at and datetime.utcnow() < opened_at + timeout:
            error_msg = 'Timeout not elapsed yet, circuit breaker will remain still open'
...
class CircuitHalfOpenState(CircuitBreakerState):
    ...
    def __init__(self, cb, prev_state=None, notify=False):
        ...
        if notify:
            print(self._breaker._listeners)
    ...
    def on_failure(self, exc):
        ...
        """
        self._breaker.open()
        print 'on_failure:  CircuitHalfOpenState'
        error_msg = 'Trial call failed, now circuit breaker opened'
        six.reraise(CircuitBreakerError, CircuitBreakerError(error_msg), sys.exc_info()[2])
    ...
    def on_success(self):
        ...
        Closes the circuit breaker.
        """
        print 'on_success:  CircuitHalfOpenState'
        print self._breaker._state_storage.fail_counter
        print self._breaker._state_storage.success_counter
        print self._breaker._state_storage.total_calls_counter
